    ...
    <div className={`${styles.flexCenter} flex-wrap md:w-full z-[10]`}>
      {zones.map((zone) => (
        <div key={zone.id} className={`flex-1 ${styles.flexCenter} min-w-[270px] my-1`}>
          { zone.id == "client-1" ? <Image src={capitalFederal} alt="zones" loading="lazy" width={zone.width} height={53} className=""/>
          : zone.id == "client-2" ? <Image src={zonaSur} alt="zones" loading="lazy" width={zone.width} height={53} className=""/>
