...
import { FaShieldAlt, FaStar, FaThumbsUp } from "react-icons/fa";
...
const FeatureCard = ({ title, content, index }: FeatureCardProps) => (
  <div
  ...
  >
    ...
    >
      {index == 1 ? (
        <FaShieldAlt className="text-secondary text-[28px]" />
      ) : index == 2 ? (
        <FaThumbsUp className="text-secondary text-[28px]" />
      ) : (
        <FaStar className="text-secondary text-[28px]" />
      )}
    </div>
...
const Features = () => (
  <section
    id="features"
    className={`${layout.section} px-6 xl:px-0 mt-6 mb:mt-0 relative z-40 features-section lg:-mt-6 xl:mt-6`}
  >
    <Reveal variant="text" className={layout.sectionInfo}>
      <h2 className={styles.heading2}>
        ¿Por qué elegirnos? <br className="block" />
        30 Años de Experiencia
      </h2>
      <p className={`${styles.paragraph} max-w-[550px] mt-5`}>
        Con más de 30 años de experiencia en el rubro brindamos una asistencia
