...
import type { FeedbackItem } from "../constants/index";
...
type FeedbackCardProps = Pick<
  FeedbackItem,
  "content" | "name" | "title" | "rating" | "id"
>;
...
const FeedbackCard = ({
  ...
  <div
    className={`flex flex-col justify-between px-4 py-6 md:px-8 md:py-10 rounded-[20px] w-[320px] md:w-[400px] h-full md:min-h-[300px] feedback-card ${
      id === "feedback-1" ? "feedback-card-set" : ""
    }`}
  >
    <p className="font-poppins font-normal text-[16px] leading-[32px] text-white my-6 flex-1">
      {content}
    </p>
    ...
    <div className="flex flex-row items-center mt-auto justify-between">
      <div className="flex">
        ...
        <div className="flex flex-col ml-4">
          <h3 className="font-poppins font-semibold text-md md:text-[16px] leading-[32px] text-white">
            {name}
          </h3>
          <p className="font-poppins font-normal text-sm md:text-[14px] leading-[24px] text-dimWhite -mt-1">
            {title}
          </p>
        </div>
      </div>
